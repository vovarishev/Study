...
int main() {
	...
	cout << "\nДанная программа  способна решать только квадратные матрицы методом СЛАУ и находить для них определитель.\n";
	//-----------------------------------------------------------------------------//Определение матриц
...
bool TriangMatr(double** A, double** B, double& determ, int n) {// Триангуляция матрицы + Определитель
	...
	for (unsigned int k = 0; k < n; k++) for (unsigned int i = k + 1; i < n; i++) {// Триангуляция построчно
		if (B[k][k] == 0 && B[i][k] != 0) {// Смена строк
			for (unsigned int j = 0; j < n + 1; j++) {
				swap(B[k][j], B[i][j]);
				determ *= -1;
			}
		}
		else if (B[k][k] == 0 && B[k][i] != 0) {// Смена столбцов
...
bool Solve(double** A, double** B, double* x, double& determ, int n) {// Решение по Гаусу
	...
	if (TriangMatr(A, B, determ, n) == true) {//Подсчёт вектора-решения
		...
		for (unsigned int i = 0; i < n; i++) {//Проверка на нулевые строки
			...
			if (zeroCheck == 0) {
				cout << "\nМатрица является вырожденной или имеет нулевые строки\n";
				return false;
			}
		}
